...
from typing import cast, Any, Callable, Container, Counter, Dict, Optional, \
    Tuple, TypeVar, Iterator, Hashable
...
class FileWatcher:
    ...
    class AssetChangedHandler(watchdog.events.FileSystemEventHandler):
        ...
        def __init__(self,
                     ...
                     on_event: Callable[[watchdog.events.FileSystemEvent], None]) -> None:
            super().__init__()
        ...
        def dispatch(self, event: watchdog.events.FileSystemEvent) -> None:
            ...
            if path.parent in self.directories and \
               path.name in self.directories[path.parent].filenames:
                self.on_event(event)
    ...
    def __init__(self, on_event: Callable[[watchdog.events.FileSystemEvent], None]) -> None:
